...
def regen_gl():
	...
	total=len(sinv)
	for doc in sinv:
		repair_only_gl_entry("Sales Invoice Penjualan Motor", doc[0])
		n=n+1
		print("Repaired {} of {}".format(n,total))
def revert_sipm():
	#sinv=frappe.db.sql("""select name,cost_center,set_warehouse,modified from `tabSales Invoice Penjualan Motor` where left(set_warehouse,7)!=left(cost_center,7) and docstatus=1 
	sinv=frappe.db.sql("""select name,cost_center,set_warehouse,modified from `tabSales Invoice Penjualan Motor` where docstatus=1
		and name IN ("ACC-SINVM-2022-10841","ACC-SINVM-2022-10865") """,as_list=1)
	cond=""
	for doc in sinv:
		sipm=frappe.get_doc("Sales Invoice Penjualan Motor",doc[0])
		if sipm.docstatus==1:
			sipm.cancel()
		if cond=="":
			cond='"{}"'.format(doc[0])
		else:
			cond='{},"{}"'.format(cond,doc[0])
	frappe.db.sql("""update `tabSales Invoice Penjualan Motor` set docstatus=0 where name in ({}) """.format(cond))
	frappe.db.sql("""update `tabTable Discount` set docstatus=0 where name in ({}) """.format(cond))
	frappe.db.sql("""update `tabTable Disc Leasing` set docstatus=0 where name in ({}) """.format(cond))
	frappe.db.sql("""update `tabSales Invoice Penjualan Motor Item` set docstatus=0 where name in ({}) """.format(cond))
	frappe.db.sql("""update `tabSales Taxes and Charges` set docstatus=0 where name in ({}) """.format(cond))
	frappe.db.sql("""update `tabTabel Biaya Motor` set docstatus=0 where name in ({}) """.format(cond))
def cancel_sinv_test():
	inv="SINV-SB-2022-06-00076"
...
def repair_gl_entry(doctype,docname):
	...
	if len(cek_apakah_tujuan) > 0:
		...
		if docu.doctype in ['Purchase Receipt','Stock Reconciliation','Stock Entry','Delivery Note','Sales Invoice Penjualan Motor']:
			repair_gl_sle_entry(doctype, docname)
		if docu.doctype in ['Purchase Invoice','Sales Invoice']:
			if docu.update_stock == 0:
				repair_only_gl_entry(doctype, docname)
			else:
				repair_gl_sle_entry(doctype, docname)
