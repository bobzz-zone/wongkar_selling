...
class calculate_taxes_and_totals_custom(object):
	...
	def calculate(self):
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice","Sales Invoice Penjualan Motor"]:
	...
	def calculate_totals(self):
		...
		if self.doc.get("tabel_biaya_motor"):
			if len(self.doc.get("tabel_biaya_motor")) > 0:
				for i in self.doc.get("tabel_biaya_motor"):
					if i.type in ['STNK','BPKB']:
						total_biaya_tanpa_dealer += i.amount

		ppn_rate = self.doc.taxes[0].rate
		print(ppn_rate, ' ppn_rate')
		ppn_div = (100+ppn_rate)/100

		total_diskon_setelah_pajak = 0
		if self.doc.get("table_discount"):
			if len(self.doc.get("table_discount"))> 0:
				for i in self.doc.get("table_discount"):
		...
		if self.doc.get("table_discount_leasing"):
			if len(self.doc.get("table_discount_leasing")) > 0:
				for i in self.doc.get("table_discount_leasing"):
		...
		nominal_diskon_sp = self.doc.nominal_diskon / ppn_div
		print(nominal_diskon_sp, ' nominal_diskon_sp')
		pajak_diskon_sp = self.doc.nominal_diskon - nominal_diskon_sp
	...
	def is_internal_invoice(self):
		...
		if self.doc.doctype in ("Sales Invoice", "Purchase Invoice","Sales Invoice Penjualan Motor") and self.doc.is_internal_transfer():
			return True
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice","Sales Invoice Penjualan Motor"]:
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice","Sales Invoice Penjualan Motor"]:
			...
			if self.doc.doctype in ["Sales Invoice","Sales Invoice Penjualan Motor"] and not self.doc.get("is_return"):
			...
			if (
				self.doc.doctype in ["Sales Invoice","Sales Invoice Penjualan Motor"]
