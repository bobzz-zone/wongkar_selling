...
class PembayaranCreditMotor(Document):
	def set_status(self):
		if self.docstatus == 2:
	...
	def make_gl_credit(self, gl_entries):
		...
		account = frappe.get_value("Company",{"name" : self.company}, "default_receivable_account")
		...
		for d in self.get('daftar_credit_motor'):
			cost_center = frappe.get_value("Company",{"name" : d.no_invoice}, "round_off_cost_center")
			gl_entries.append(
				self.get_gl_dict({
					"account": account,
					"party_type": "Customer",
	...
	def make_gl_debit(self, gl_entries):
		...
		data = frappe.db.sql(""" SELECT SUM(nilai) AS nilai,cost_center FROM `tabDaftar Credit Motor` cd
	...
	def on_submit(self):
		...
		for d in self.get('daftar_credit_motor'):
			outstanding_amount = frappe.get_value("Sales Invoice Penjualan Motor",{"name" : d.no_invoice}, "outstanding_amount")
			doc = frappe.get_doc('Sales Invoice Penjualan Motor',{'name': d.no_invoice})
			doc.outstanding_amount = d.nilai - outstanding_amount
			doc.status='Paid'
			doc.db_update()
			frappe.db.commit()
	...
	def on_cancel(self):
		...
		for d in self.get('daftar_credit_motor'):
			...
			frappe.db.commit()
			# frappe.msgprint('Berhasil !')
			# data = frappe.db.get_list('GL Entry',filters={'parent': self.name},fields=['*'])
		self.set_status()
...
def add_party_gl_entries_custom(self):
	account = frappe.get_value("Company",{"name" : self.company}, "default_receivable_account")
	cost_center = frappe.get_value("Company",{"name" : self.company}, "cost_center")
	# for d in self.get('daftar_credit_motor'):
	# 	nama_customer = frappe.get_value("Sales Invoice Penjualan Motor",{"name" : d.no_invoice}, "customer")
	# 	account = frappe.get_value("Sales Invoice Penjualan Motor",{"name" : d.no_invoice}, "debit_to")
	# 	cost_center = frappe.get_value("Sales Invoice Penjualan Motor",{"name" : d.no_invoice}, "cost_center")
	# 	name = frappe.get_value("Sales Invoice Penjualan Motor",{"name" : d.no_invoice}, "name")
	# 	mydate= datetime.date.today()
	# 	docgl = frappe.new_doc('GL Entry')
	# 	docgl.posting_date = date.today()
	# 	docgl.party_type = "Customer"
	# 	docgl.party = nama_customer
	# 	docgl.account = account
	# 	docgl.cost_center = cost_center
	# 	docgl.credit = d.nilai
	# 	docgl.credit_in_account_currency = d.nilai
	# 	docgl.account_currency = "IDR"
	# 	docgl.against = account
	# 	docgl.against_voucher_type = "Sales Invoice Penjualan Motor"
	# 	docgl.against_voucher = name
	# 	docgl.voucher_type = "Pembayaran Credit Motor"
	...
	docgl.account_currency = "IDR"
	docgl.against = self.coa_credit_motor
	docgl.voucher_type = "Pembayaran Credit Motor"
	docgl.voucher_no = self.name
	docgl.remarks = "No Remarks"
	docgl.is_opening = "No"
...
def add_party_gl_entries_custom_tambah(self):
	# account = frappe.get_value("Rule",{"name" : self.customer}, "coa_receivable")
	cost_center = frappe.get_value("Company",{"name" : self.company}, "round_off_cost_center")
	...
	docgl.party_type = "Customer"
	docgl.party = self.customer
	docgl.account = self.coa_credit_motor
	docgl.cost_center = cost_center
