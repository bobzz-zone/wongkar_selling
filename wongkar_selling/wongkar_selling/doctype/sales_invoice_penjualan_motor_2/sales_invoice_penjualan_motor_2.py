...
class SalesInvoicePenjualanMotor2(SalesInvoice):
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				...
				if self.doctype == "Sales Invoice Penjualan Motor 2" and self.docstatus == 1:
	...
	def on_submit(self):
		...
		if not self.is_return and not self.is_consolidated and self.loyalty_program:
			...
			against_si_doc = frappe.get_doc("Sales Invoice Penjualan Motor 2", self.return_against)
	...
	def check_if_consolidated_invoice(self):
		...
		if self.doctype == "Sales Invoice Penjualan Motor 2" and self.is_consolidated:
	...
	def update_status_updater_args(self):
		if cint(self.update_stock):
			self.status_updater.append({
				...
				'extra_cond': """ and exists(select name from `tabSales Invoice Penjualan Motor 2`
			...
			if cint(self.is_return):
				self.status_updater.append({
					...
					'second_join_field': 'so_detail',
					'extra_cond': """ and exists (select name from `tabSales Invoice Penjualan Motor 2` where name=`tabSales Invoice Penjualan Motor Item`.parent and update_stock=1 and is_return=1)"""
				})
	...
	def validate_serial_against_delivery_note(self):
		"""
			validate if the serial numbers in Sales Invoice Penjualan Motor Items are same as in
			Delivery Note Item
		"""
	...
	def get_returned_amount(self):
		returned_amount = frappe.db.sql("""
			...
			from `tabSales Invoice Penjualan Motor 2`
	...
	def set_healthcare_services(self, checked_values):
		...
		for checked_item in checked_values:
			...
			args = {
				'doctype': "Sales Invoice Penjualan Motor 2",
	...
	def set_status(self, update=False, status=None, update_modified=True):
		...
		if not status:
			if self.docstatus == 2:
				...
				elif outstanding_amount <= 0 and self.is_return == 0 and frappe.db.get_value('Sales Invoice Penjualan Motor 2', {'is_return': 1, 'return_against': self.name, 'docstatus': 1}):
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order"]:
		partytype, ref_partytype, internal = "Customer", "Supplier", "is_internal_customer"

		if doctype == "Sales Invoice Penjualan Motor 2":
		...
		if doctype == "Purchase Invoice":
			ref_doc = "Sales Invoice Penjualan Motor 2"
		else:
			ref_doc = "Sales Order"

	if inter_company_reference:
		doc = frappe.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order"] else doc.customer
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if doctype in ["Sales Invoice Penjualan Motor 2", "Purchase Invoice"]:
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if doctype in ["Sales Invoice Penjualan Motor 2", "Purchase Invoice"]:
		ref_doc = "Purchase Invoice" if doctype == "Sales Invoice Penjualan Motor 2" else "Sales Invoice Penjualan Motor 2"
		ref_field = "inter_company_invoice_reference"
	else:
...
def make_maintenance_schedule(source_name, target_doc=None):
	doclist = get_mapped_doc("Sales Invoice Penjualan Motor 2", source_name, 	{
		"Sales Invoice Penjualan Motor 2": {
...
def make_sales_return(source_name, target_doc=None):
	...
	return make_return_doc("Sales Invoice Penjualan Motor 2", source_name, target_doc)
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order", "Delivery Note"]:
...
def validate_inter_company_transaction(doc, doctype):
	...
	price_list = doc.selling_price_list if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order", "Delivery Note"] else doc.buying_price_list
	...
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order"] else "Customer"
...
def make_inter_company_purchase_invoice(source_name, target_doc=None):
	return make_inter_company_transaction("Sales Invoice Penjualan Motor 2", source_name, target_doc)

def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice Penjualan Motor 2", "Sales Order"]:
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Purchase Invoice" if doctype == "Sales Invoice Penjualan Motor 2" else "Purchase Order"
		target_detail_field = "sales_invoice_item" if doctype == "Sales Invoice Penjualan Motor 2" else "sales_order_item"
		source_document_warehouse_field = 'target_warehouse'
		target_document_warehouse_field = 'from_warehouse'
	else:
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Sales Invoice Penjualan Motor 2" if doctype == "Purchase Invoice" else "Sales Order"
...
def on_doctype_update():
	frappe.db.add_index("Sales Invoice Penjualan Motor 2", ["customer", "is_return", "return_against"])

@frappe.whitelist()
def create_invoice_discounting(source_name, target_doc=None):
	invoice = frappe.get_doc("Sales Invoice Penjualan Motor 2", source_name)
