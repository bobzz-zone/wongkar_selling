...
class TagihanDiscountLeasing(Document):
	def cal_outstanding_pph(self):
		for i in self.daftar_tagihan_leasing:
			rate = frappe.get_doc('Sales Taxes and Charges',{'parent':i.no_invoice,'idx':1}).rate
			tax = (100+rate ) / 100
			pjk = i.nilai / tax
			pph = pjk * (self.pph/100)
			if i.nilai == 1000000:
				print(f"{i.nilai} - {pph}")
			outstnding = i.nilai - pph
			i.outstanding_discount = outstnding
	...
	def hitung_pph(self):
		...
		for d in self.daftar_tagihan_leasing:
			...
			pjk = d.nilai / tax
			pph = pjk * (self.pph/100)
			d.outstanding_discount = flt(d.nilai - pph,0)
			total +=  flt(d.nilai - pph,0)
		# print(total, ' total')
			...
			for d2 in account:
				...
				gl_entries.append(
					self.get_gl_dict({
						...
						"debit": flt(d2.nominal,0),
						"debit_in_account_currency": flt(d2.nominal,0),
		...
		for d in self.get('daftar_tagihan_leasing'):
			...
			gl_entries.append(
				self.get_gl_dict({
					...
					"debit": flt(pph,0),
					"debit_in_account_currency":flt(pph,0),
					# "cost_center": cost_center,
			...
			for d2 in account:
				gl_entries.append(
					self.get_gl_dict({
						...
						"credit": flt(hitung_tax,0),
						"credit_in_account_currency": flt(hitung_tax,0),
						"against_voucher": d.no_invoice,
						"against_voucher_type": "Sales Invoice Penjualan Motor",
						"cost_center": cost_center
						# "project": self.project,
		...
		for d in self.get('daftar_tagihan_leasing'):
			...
			gl_entries.append(
				self.get_gl_dict({
					...
					"against": self.customer,
					"credit": flt(net,0),
					"credit_in_account_currency": flt(net,0),
					# "cost_center": cost_center,
					# "remarks": "coba Lutfi pajak!"
				}, item=None)
			)
			...
			# print(sum(total), " total")
				...
				gl_entries.append(
					self.get_gl_dict({
						...
						"credit": flt(d2.nominal,0),
						"credit_in_account_currency": flt(d2.nominal,0),
						"against_voucher": d.no_invoice,
						"against_voucher_type": "Sales Invoice Penjualan Motor",
						# "cost_center": cost_center
						# "project": self.project,
	...
	def make_gl_debit(self, gl_entries):
		...
		data = frappe.db.sql(""" SELECT SUM(tdl.nominal) AS nilai,cost_center,tdl.coa,sinv.name FROM `tabDaftar Tagihan Leasing` cd
			...
			WHERE cd.parent = '{}' and tdl.nama_leasing = '{}' GROUP BY tdl.nama_leasing,sinv.name """.format(self.name,self.customer),as_dict=1)
		...
		for d in data:
			...
			# net  = d.nilai - pph
			print(d.nilai,flt(d.nilai - pph,0), ' sdfsdf')
			gl_entries.append(
				self.get_gl_dict({
					...
					"debit": flt(d.nilai - pph,0),
					"debit_in_account_currency": flt(d.nilai - pph,0),
