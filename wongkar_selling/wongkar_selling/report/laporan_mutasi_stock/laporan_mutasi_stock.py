...
def get_sn(filters):
	...
	if filters.get("item_code"):
		conditions = 'and sn.item_code = "{}"'.format(filters.get("item_code"))
	...
	data_sn = frappe.db.sql(""" 
		SELECT 
			...
			0 as opening_val,
			0 as opening_qty,
			sn.purchase_rate as in_val,
			1 as in_qty,
			0 as out_qty,
			sn.purchase_rate as bal_val
		from `tabSerial No` sn where sn.status = 'Active' {} and sn.purchase_date between '{}' and '{}' """.format(conditions,from_date,to_date),as_dict=1,debug=0)
