...
class ReceivablePayableReport(object):
	...
	def set_party_details(self, row):
		...
		# frappe.msgprint(str(party_details) + ' party_details')
		if self.filters.get(scrub(self.filters.party_type)):
			row.currency = row.account_currency
			row.nama_vendor = party_details['supplier_name']
		else:
	...
	def get_lpb(self,row):
		if row.voucher_type == 'Purchase Invoice':
			...
			for i in po:
				prec = frappe.db.sql(""" 
					SELECT 
						pri.name,pri.parent,pri.item_code,pri.serial_no,prec.posting_date
					from `tabPurchase Receipt` prec 
					join `tabPurchase Receipt Item` pri on pri.parent = prec.name
					where prec.docstatus = 1 and pri.purchase_order = '{}' and pri.purchase_order_item = '{}' order by prec.posting_date desc """.format(i['purchase_order'],i['po_detail']),as_dict=1)

			if prec:
				row['tgl_lpb'] = prec[0]['posting_date']
	...
	def get_ageing_data(self, entry_date, row):
		...
		if not (self.age_as_on and entry_date):
			return
		# frappe.msgprint(str(self.age_as_on)+ ' dfdf')
	...
	def get_columns(self):
		...
		if self.party_type == "Supplier":
			...
			self.add_column(label=_("Tanggal Invoice"), fieldname="bill_date", fieldtype="Date")
			self.add_column(label=_("Tanggal LPB"), fieldname="tgl_lpb", fieldtype="Date")
			
		self.add_column(
			label=_('Tanggal Record Inputan'),
		...
		self.add_column(_("Utang"), fieldname="outstanding")
	...
	def setup_ageing_columns(self):
		...
		self.add_column(label=_("Belum Tempo"), fieldname="blm_tempo", fieldtype="Currency", width=100)
