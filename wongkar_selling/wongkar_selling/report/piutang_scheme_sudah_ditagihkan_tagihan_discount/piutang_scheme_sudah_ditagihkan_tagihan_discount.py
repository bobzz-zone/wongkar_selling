...
def get_data(filters):
	...
	data = frappe.db.sql(""" SELECT 
			sipm.name AS sipm,
			t_disc.`customer` AS customer,
			sipm.nama_pemilik,
			sipm.posting_date AS tgl_jual,
			sipm.cost_center AS nama_area,
			dt.`terbayarkan` AS outstanding_amount,
			dt.`nilai`,
			dt.terbayarkan,
			...
			AND sipm.posting_date <= '{}' AND dt.`terbayarkan` > 0 and t_disc.`name` IS not NULL
			{}
			GROUP BY sipm.name,t_disc.`name` ORDER BY sipm.posting_date ASC
