...
def get_data(filters):
	...
	data = frappe.db.sql(""" SELECT 
			...
			LEFT JOIN `tabDaftar Tagihan Leasing` dt ON dt.`no_invoice` = sipm.name
			LEFT JOIN `tabTagihan Discount Leasing` t_disc ON t_disc.`name` = dt.`parent` AND t_disc.`docstatus` = 1
			LEFT JOIN `tabTagihan Payment Table` tp ON tp.`doc_name` = t_disc.name AND dt.`no_invoice` = tp.no_sinv AND tp.`docstatus` = 1
			...
			AND sipm.posting_date <= '{}' AND td.`nominal` > 0 and t_disc.`name` IS not NULL
			{}
